...
```bash
npm install autoprefixer postcss tailwindcss lodash-es
```

Create PostCSS and Tailwind config files:

```bash
echo 'import tailwindConfig from "@xola/ui-kit/tailwind.config";\nexport default tailwindConfig;' > tailwind.config.js
echo 'import postcss from "@xola/ui-kit/postcss.config";\nexport default postcss;' > postcss.config.js
```

Import main CSS files in your project:

```js
import "@xola/ui-kit/ui-kit-lib.css";
import "@xola/ui-kit/ui-kit-theme.css";
```
